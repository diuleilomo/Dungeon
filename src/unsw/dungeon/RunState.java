...
public class RunState implements EnemyState {
	...
	public RunState(Enemy enemy, Dungeon dungeon) {
		this.enemy = enemy;
	...
	public void move() {
		...
		// two way scenario
			...
			if (move[0] == true && move[2] == true) {
				if (ydiff > 0) {this.enemy.moveDown(); return;}
				else {this.enemy.moveUp(); return;}
			} else if (move[1] == true && move[3] == true) {
				if (xdiff > 0) {this.enemy.moveRight(); return;}
				else {this.enemy.moveLeft(); return;}
			} else {
				if (ydiff < xdiff) {
					if (move[0] == true) {this.enemy.moveUp(); return;}
					if (move[2] == true) {this.enemy.moveDown(); return;}
	...
	public boolean[] canMove(boolean[] array) {
		...
		/*
		if (temp[0] != null) {
			System.out.println("enemy up: " + temp[0].getClass().toString());
		}
		if (temp[1] != null) {
			System.out.println("enemy right: " + temp[1].getClass().toString());
		}
		if (temp[2] != null) {
			System.out.println("enemy down: " + temp[2].getClass().toString());
		}
		if (temp[3] != null) {
			System.out.println("enemy left: " + temp[3].getClass().toString());
		}
		*/
